...
try:
    ...
    rescore_pipeline_result_for_bortle = getattr(
        backend_module,
        "rescore_pipeline_result_for_bortle",
        None,
    )
...
include_positions = st.sidebar.checkbox(
    ...
    value=False,
    help="Adds hourly Sun/Moon position data for Sky Path and, when available, improves the moonlight scoring penalty.",
)
...
if st.sidebar.button("Clear cached data", width="stretch"):
    ...
    st.session_state.pop("pipeline_inputs", None)
...
def build_score_credibility_audit(
    ...
    score_range = validation_df["stargazing_score"].max() - validation_df["stargazing_score"].min()
    seeing_std = (
...
def render_score_validation_panel(score_df: pd.DataFrame, bortle_index: int, result: dict = None):
    ...
    render_audit_cards(audit_df)

    dist_tab, factor_tab, sensitivity_tab, guidance_tab = st.tabs(
        [
            "Score Distributions",
    ...
    with guidance_tab:
        render_score_improvement_guidance(validation_df, credibility_issues)
...
def render_source_badges(result):
    ...
    has_tad_events = event_df is not None and not event_df.empty
    ...
    tad_badge = (
        '<span class="source-pill">Events: Timeanddate</span>'
        if has_tad_events
        else ''
    )
    st.markdown(
        ...
        f'<span class="{a_cls}">Moon/Twilight base: {astronomy_source}</span>'
        f'{tad_badge}'
...
def _night_timeline_card_html(lat: float, lon: float) -> str:
    ...
    times = {k: sky.get(k, fallback[k]) for k in fallback}
    ...
    labels_html = ""
    n = len(phases)
    for i, (icon, lbl, time_s, pct) in enumerate(phases):
        if not (lbl in KEY or all(abs(pct - kp) >= MINGAP for kp in key_pcts)):
            continue
        if i == 0:
            tx, align = "translateX(0%)",    "flex-start"
        elif i == n - 1:
            tx, align = "translateX(-100%)", "flex-end"
        else:
    ...
    return (
        ...
        f'{ticks_html}</div>'
        f'{labels_html}'
        f'<div id="nd-now-cursor" style="position:absolute;top:-6px;'
...
if run_button:
    with st.spinner("Fetching live weather and astronomy data..."):
        try:
            ...
            previous_inputs = st.session_state.get("pipeline_inputs", {})
            current_inputs = {
                "city_name": city_name,
            ...
            can_rescore_existing = (
                previous_result is not None
                and previous_inputs == current_inputs
                and rescore_pipeline_result_for_bortle is not None
            )

            if can_rescore_existing:
                result = rescore_pipeline_result_for_bortle(
                    previous_result,
                    bortle_index=bortle_index,
                )
            else:
            ...
            st.session_state["pipeline_inputs"] = current_inputs
